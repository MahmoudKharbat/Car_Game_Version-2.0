...
//            moving the phone in z axis will move the phone back and forward
//            so if the z was positive we are moving the phone backward so we have to make the stone,coin move slower
//            and if the z was negative then we are moving the phone forward so we have to make the stone,coin move faster
            if(z>2)
                y=1;
    ...
    public void startSensorsGame(){ // Function that is responsible about moving the stone in the tracks
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if(sensorsGame.getStone().getParent() == sensorsGame.getCar().getParent() && (stonePlace > carPlace-50 && stonePlace < carPlace+50)){ // If the stone touches the car then there is a crash
                            ...
                            sensorsGame.getStone().setY(stonePlace + y); // Move the stone in the y axis
                            sensorsGame.getCoin().setY(coinPlace + y); // Move the coin in the y axis
